...
class MALT90Source():
    ...
    def __init__(self,atlasgal_id,goodname,velocity,cat_row,source_lons,source_lats,source_ids):
        ...
        self.data_dir = malt.base
        self.result_dir = malt.base+'results/'
    ...
    def make_moment_images(self,linelist,tag,format='.png',mom='mom0',secondvel=False,outdir="Images"):
        ...
        for i,line in enumerate(linelist):
            ...
            gc = aplpy.FITSFigure(fname,figure=fullimage,
                   subplot=[xoff+(i%n)*xsize,yoff+(i//n)*ysize,xsize,ysize],auto_refresh=False)
            ...
            if mainline:
                gc.add_label(0.03,0.93,latex_dict[line],fontsize=16,relative=True,
                             color='black',backgroundcolor=(1,1,1,0.7),horizontalalignment='left',alpha=0.9)
            else:
                gc.add_label(0.05,0.85,latex_dict[line],fontsize=16,relative=True,
                             color='black',backgroundcolor=(1,1,1,0.7),horizontalalignment='left',alpha=0.9)
                gc.add_label(0.05,0.70,str(round(mmax,1))+" K km/s",size=6,relative=True,
                              color='black',backgroundcolor=(1,1,1,0.7),horizontalalignment='left',alpha=0.9)
        ...
        fullimage.text(0.15,0.87,self.agal_id+" : "+self.agal_name+" : "+self.malt90_name,fontsize = 14)
    ...
    def get_path(self,line='hcop',mmap='emap',secondvel = False):
        ...
        if secondvel:
            ...
            "sources",self.name,self.name+"_v2_"+line+"_mommaps",self.name+"_v2_"+line+"_"+mmap+".fits")
        else:
            filename = os.path.join(self.data_dir,
            "mommaps",line,self.aname+"_"+line+"_mommaps",self.aname+"_"+line+"_"+mmap+".fits")
